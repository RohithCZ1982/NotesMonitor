...
export default function Modal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-fade-in" role="dialog" aria-modal="true">
      ...
      <div className={`relative bg-white rounded-2xl shadow-2xl shadow-teal-100/50 w-full ${sizeClasses[size]} animate-slide-up max-h-[90vh] flex flex-col`}>
        {(title || showClose) && (
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 shrink-0">
            ...
            {showClose && (
              <button
                onClick={onClose}
                className="ml-auto p-1.5 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-teal-50 transition-colors"
                aria-label="Close"
              >
                <X size={18} />
              </button>
        ...
        <div className="flex gap-3 mt-6">
          ...
          <button
            ...
            className={`flex-1 px-4 py-2 text-sm font-medium text-white rounded-lg transition-colors disabled:opacity-50 ${
              variant === 'danger' ? 'bg-red-500 hover:bg-red-600' : 'bg-teal-600 hover:bg-teal-700'
            }`}
          >
